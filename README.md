...
* if the map doesn't include an exit, collectible, or the players starting position.
* if the map is not rectangula.
* if the map is not enclosed by walls.

### 2. Setting up the game:
The program then takes the information obtained by the parser, initializes all the assets in the correct location and loops through each image in order to correctly place all the asset pixels to an image which is then pushed to the window, through the use of ```minilibx``` functions.  Furthermore this stage also assigns number of enemys as well as their starting positions, which is done through a combination of a random function and the number of empty tiles in the game.
